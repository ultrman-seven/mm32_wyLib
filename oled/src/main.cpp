...
int main(void)
{
    ...
    // oledSoft.RES_Pin = GPIO_Pin_5;
    // oledSoft.RES_Port = GPIOA;
    // oledSoft.DC_Pin = GPIO_Pin_10;
    // oledSoft.DC_Port = GPIOB;
    // oledSoft.SCLK_Pin = GPIO_Pin_6;
    // oledSoft.SCLK_Port = GPIOA;
    // oledSoft.MOSI_Pin = GPIO_Pin_11;
    // oledSoft.MOSI_Port = GPIOB;
    // oledSoft.CS_Pin = GPIO_Pin_4;
    // oledSoft.CS_Port = GPIOA;

    delayInit();
    OLED::OLED_Object screen(&oledSoft);
    // TFT::Tft screen(&oledSoft);
    s = &screen;
    screen.loadFont(ASCII[0], 16, 8); //装载字体
    // screen.loadFont(ASCII_24_12[0], 24, 12);
    screen.setScreenSize(128, 64); //设置屏幕分辨率
    screen.clear();
    // screen.showimage(gImage_qq);
    ...
    // printf("hello %d",100);
    screen.putNum(sizeof((uint8_t)0xff));
    screen.print("\n");
    screen.putNum(sizeof(0xffff));
    // screen.dsp_single_colour(0x0000);
    while (1)
    {
        // int i = 0;
        // while (i < 20)
